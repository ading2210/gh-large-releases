...
def file_split(path, big_chunk_size, small_chunk_size=(25*1024*1024)):
  ...
  with open(path, "rb") as read_file:
    for i in range(0, big_chunks):
      new_path = path.with_name(path.name + ".{:04}".format(i))
      ...
      with open(new_path, "wb") as write_file:
        for j in range(0, small_chunks):
          small_size = get_size(big_size, small_chunk_size, small_chunks, i)
